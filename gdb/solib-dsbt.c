...
get_dsbt_info (void)
{
  ...
  info = (struct dsbt_info *) program_space_data (current_program_space, solib_dsbt_pspace_data);
  if (info != NULL)
    return info;
...
dsbt_current_sos (void)
{
  ...
  while (lm_addr)
      ...
      if (dsbt_index != 0)
	{
	  ...
	  sop = (struct so_list *) xcalloc (1, sizeof (struct so_list));
	  sop->lm_info = (struct lm_info *) xcalloc (1, sizeof (struct lm_info));
...
dsbt_relocate_main_executable (void)
{
  ...
  info->main_executable_lm_info = (struct lm_info *) xcalloc (1, sizeof (struct lm_info));
  info->main_executable_lm_info->map = ldm;

  new_offsets = (struct section_offsets *) xcalloc (symfile_objfile->num_sections,

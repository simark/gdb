...
{
  VEC(gdb_xml_value_s) **values = (struct VEC_gdb_xml_value_s **) data;
  struct gdb_xml_value *value;
  int ix;
...
{
  struct gdb_xml_parser *parser = (struct gdb_xml_parser *) XML_GetUserData (expat_parser);
...
gdb_xml_parse_attr_enum (struct gdb_xml_parser *parser,
			 ...
			 const char *value)
{
  const struct gdb_xml_enum *enums = (const struct gdb_xml_enum *) attribute->handler_data;
  void *ret;

  for (enums = (const struct gdb_xml_enum *) attribute->handler_data; enums->name != NULL; enums++)
...
xml_xinclude_default (void *data_, const XML_Char *s, int len)
{
  ...
  struct xinclude_parsing_data *data = (struct xinclude_parsing_data *) parser->user_data;

		 ...
		 int copy_names)
{
  ...
  for (i = 0; i < number_of_symbols; i++)
      ...
      if (sym->flags & BSF_FILE)
	{
	  ...
	  filesym = sym;
	  filesymname = (const char *) bcache (filesym->name, strlen (filesym->name) + 1,
...
{
  const struct elf_gnu_ifunc_cache *a = (const struct elf_gnu_ifunc_cache *) a_voidp;
...
{
  const struct elf_gnu_ifunc_cache *a = (const struct elf_gnu_ifunc_cache *) a_voidp;
  const struct elf_gnu_ifunc_cache *b = (const struct elf_gnu_ifunc_cache *) b_voidp;
...
elf_gnu_ifunc_record_cache (const char *name, CORE_ADDR addr)
{
  ...
  entry_p = (struct elf_gnu_ifunc_cache *) obstack_finish (&objfile->objfile_obstack);

  slot = htab_find_slot (htab, entry_p, INSERT);
  if (*slot != NULL)
    {
      struct elf_gnu_ifunc_cache *entry_found_p = (struct elf_gnu_ifunc_cache *) *slot;
      struct gdbarch *gdbarch = get_objfile_arch (objfile);
...
elf_gnu_ifunc_resolve_by_cache (const char *name, CORE_ADDR *addr_p)
{
  ...
  ALL_PSPACE_OBJFILES (current_program_space, objfile)
	...
	continue;
      ...
      entry_p = (struct elf_gnu_ifunc_cache *) alloca (sizeof (*entry_p) + strlen (name));
...
elf_get_probes (struct objfile *objfile)
{
  ...
  probes_per_bfd = (struct VEC_probe_p *) bfd_data (objfile->obfd, probe_key);
...
{
  int ix;
  VEC (probe_p) *probes = (struct VEC_probe_p *) d;
  struct probe *probe;

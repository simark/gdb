...
static enum record_stop_reason record_full_stop_reason
                ...
                else
		  {
			...
			(get_regcache_aspace (regcache),
			 entry->u.mem.addr, entry->u.mem.len))
		      record_full_stop_reason = RECORD_STOPPED_BY_WATCHPOINT;
		  }
              }
          }
      }
		    ...
		    int options)
{
  ...
  record_full_stop_reason = RECORD_STOPPED_BY_NO_REASON;
      ...
      else
	{
	  ...
	  while (1)
		  ...
		  && status->value.sig == GDB_SIGNAL_TRAP)
		{
		  ...
		  enum record_stop_reason *stop_reason_p
      ...
      do
	{
	  ...
	  if (record_full_list->type == record_full_end)
	      ...
	      else
		{
		  ...
		  if (record_full_stop_reason == RECORD_STOPPED_BY_WATCHPOINT)
		    {
		      if (record_debug)
			fprintf_unfiltered (gdb_stdlog,
...
{
  if (RECORD_FULL_IS_REPLAY)
    return record_full_stop_reason == RECORD_STOPPED_BY_WATCHPOINT;
  else
...
{
  return record_full_stop_reason == RECORD_STOPPED_BY_SW_BREAKPOINT;
}
...
{
  return record_full_stop_reason == RECORD_STOPPED_BY_HW_BREAKPOINT;
}

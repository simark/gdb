		   ...
		   struct gdbarch *gdbarch)
{
  ...
  ps->expout_ptr = 0;
  ps->expout = (struct expression *) xmalloc (sizeof (struct expression)
			  ...
			  int *oplenp, int *argsp)
{
  ...
  switch (i)
    ...
    case OP_F90_RANGE:
      ...
      range_type
	= (enum f90_range_type) longest_to_int (expr->elts[endpos - 2].longconst);
      switch (range_type)
	{
...
type_stack_reserve (struct type_stack *stack, int howmuch)
{
  if (stack->depth + howmuch >= stack->size)
	...
	stack->size = howmuch;
      stack->elements = (union type_stack_elt *) xrealloc (stack->elements,

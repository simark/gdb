...
add_tracepoint_action (struct tracepoint *tpoint, char *packet)
{
  ...
  while (*act)
	...
	case 'M':
	  {
	    ...
	    maction = (struct collect_memory_action *) xmalloc (sizeof *maction);
	...
	case 'R':
	  {
	    ...
	    raction = (struct collect_registers_action *) xmalloc (sizeof *raction);
	...
	case 'L':
	  {
	    ...
	    raction = (struct collect_static_trace_data_action *) xmalloc (sizeof *raction);
	    raction->base.type = *act;
      ...
      if (seen_step_action_flag)
	{
	  ...
	  tpoint->step_actions
	    = (struct tracepoint_action **) xrealloc (tpoint->step_actions,
      ...
      else
	{
	  tpoint->numactions++;
	  tpoint->actions
	    = (struct tracepoint_action **) xrealloc (tpoint->actions,
...
create_trace_state_variable (int num, int gdb)
{
  ...
  tsv = (struct trace_state_variable *) xmalloc (sizeof (struct trace_state_variable));
  tsv->number = num;
  tsv->initial_value = 0;
  tsv->value = 0;
  tsv->getter = NULL;
  ...
  while (*packet == ':')
    {
      ...
      roreg = (struct readonly_region *) xmalloc (sizeof (struct readonly_region));
      roreg->start = start;
      roreg->end = end;
  ...
  if (startswith (stop_reason_rsp, "terror:"))
    {
      ...
      p = stop_reason_rsp = (char *) alloca (strlen ("terror:") + hexstr_len + 1);
      strcpy (p, "terror:");
      p += strlen (p);

...
{
  struct dwarf2_locexpr_baton *symbaton = (struct dwarf2_locexpr_baton *) SYMBOL_LOCATION_BATON (framefunc);
...
loclist_find_frame_base_location (struct symbol *framefunc, CORE_ADDR pc,
				  const gdb_byte **start, size_t *length)
{
  struct dwarf2_loclist_baton *symbaton = (struct dwarf2_loclist_baton *) SYMBOL_LOCATION_BATON (framefunc);
  ...
  if (result == NULL)
    {
      ...
      result = (struct call_site_chain *) xmalloc (sizeof (*result) + sizeof (*result->call_site)
					   * (length - 1));
			  ...
			  CORE_ADDR *value)
{
  ...
  switch (prop->kind)
    ...
    case PROP_LOCEXPR:
      {
	const struct dwarf2_property_baton *baton = (const struct dwarf2_property_baton *) prop->data.baton;
    ...
    case PROP_LOCLIST:
      {
	struct dwarf2_property_baton *baton = (struct dwarf2_property_baton *) prop->data.baton;
...
{
  struct dwarf2_locexpr_baton *dlbaton = (struct dwarf2_locexpr_baton *) SYMBOL_LOCATION_BATON (symbol);
  struct value *val;
...
{
  struct dwarf2_locexpr_baton *dlbaton = (struct dwarf2_locexpr_baton *) SYMBOL_LOCATION_BATON (sym);
  unsigned int addr_size = dwarf2_per_cu_addr_size (dlbaton->per_cu);
...
loclist_generate_c_location (struct symbol *sym, struct ui_file *stream,
			     ...
			     CORE_ADDR pc, const char *result_name)
{
  struct dwarf2_loclist_baton *dlbaton = (struct dwarf2_loclist_baton *) SYMBOL_LOCATION_BATON (sym);
  unsigned int addr_size = dwarf2_per_cu_addr_size (dlbaton->per_cu);
  const gdb_byte *data;
  size_t size;

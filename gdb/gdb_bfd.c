...
eq_bfd (const void *a, const void *b)
{
  ...
  const struct gdb_bfd_cache_search *s = (const struct gdb_bfd_cache_search *) b;
...
{
  struct gdb_bfd_section_data *sect = (struct gdb_bfd_section_data *) bfd_get_section_userdata (abfd, sectp);
...
gdb_bfd_ref (struct bfd *abfd)
{
  ...
  gdata = (struct gdb_bfd_data *) bfd_zalloc (abfd, sizeof (struct gdb_bfd_data));
  gdata->refc = 1;
...
get_section_descriptor (asection *section)
{
  ...
  result = (struct gdb_bfd_section_data *) bfd_get_section_userdata (section->owner, section);

  if (result == NULL)
    {
      result = (struct gdb_bfd_section_data *) bfd_zalloc (section->owner, sizeof (*result));

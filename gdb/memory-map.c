...
{
  struct memory_map_parsing_data *data = (struct memory_map_parsing_data *) user_data;
  struct mem_region *r = VEC_safe_push (mem_region_s, *data->memory_map, NULL);
  ULONGEST *start_p, *length_p, *type_p;

  start_p = (long unsigned int *) xml_find_attribute (attributes, "start")->value;
  length_p = (long unsigned int *) xml_find_attribute (attributes, "length")->value;
  type_p = (long unsigned int *) xml_find_attribute (attributes, "type")->value;
...
clear_result (void *p)
{
  VEC(mem_region_s) **result = (struct VEC_mem_region_s **) p;

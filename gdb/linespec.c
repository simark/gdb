...
{
  ++sals->nelts;
  sals->sals = (struct symtab_and_line *) xrealloc (sals->sals, sals->nelts * sizeof (sals->sals[0]));
  ...
  if (self->canonical)
    {
      ...
      self->canonical_names = (struct linespec_canonical_name *) xrealloc (self->canonical_names,
					(sals->nelts
...
iterate_inline_only (struct symbol *sym, void *d)
{
  if (SYMBOL_INLINED (sym))
    {
      struct symbol_and_data_callback *cad = (struct symbol_and_data_callback *) d;
...
{
  const struct symbol_matcher_data *data = (const struct symbol_matcher_data *) d;
...
decode_objc (struct linespec_state *self, linespec_p ls, const char **argptr)
{
  ...
  if (!VEC_empty (symbolp, info.result.symbols)
      || !VEC_empty (bound_minimal_symbol_d, info.result.minimal_symbols))
    {
      ...
      saved_arg = (char *) alloca (new_argptr - *argptr + 1);
...
{
  struct decode_compound_collector *collector = (struct decode_compound_collector *) d;
  void **slot;
  struct type *t;
...
{
  const struct bound_minimal_symbol *sa = (const struct bound_minimal_symbol *) a;
  const struct bound_minimal_symbol *sb = (const struct bound_minimal_symbol *) b;
  uintptr_t uia, uib;
  ...
  if (VEC_empty (symbolp, *symbols)
      ...
      if (last == NULL)
	{
	 ...
	 LAST points to the method name.  */
      klass = (char *) xmalloc ((last - lookup_name + 1) * sizeof (char));
      ...
      method = (char *) xmalloc ((strlen (last) + 1) * sizeof (char));

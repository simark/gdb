...
{
  struct type *vtable_type = (struct type *) gdbarch_data (gdbarch, vtable_type_gdbarch_data);
...
{
  struct type *vtable_type = (struct type *) gdbarch_data (gdbarch,
...
{
  const struct value_and_voffset * const *ova = (const struct value_and_voffset * const*) a;
  CORE_ADDR addra = (value_address ((*ova)->value)
		     + value_embedded_offset ((*ova)->value));
  const struct value_and_voffset * const *ovb = (const struct value_and_voffset * const*) b;
...
gnuv3_get_typeid_type (struct gdbarch *gdbarch)
{
  ...
  if (typeinfo == NULL)
    typeinfo_type = (struct type *) gdbarch_data (gdbarch, std_type_info_gdbarch_data);
  else

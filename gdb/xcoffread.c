	...
	if (symbol->n_zeroes)
	  {
		...
		p = (char *) obstack_alloc (&objfile->objfile_obstack, E_SYMNMLEN + 1);
  ...
  if (ISFCN (cs->c_type))
    {
      ...
      SYMBOL_SET_LINKAGE_NAME (sym, (const char *) SYMNAME_ALLOC (name, symname_alloced));
	...
	if (secp)
	  {
	    ...
	    if (length)
	      {
		debugsec = (bfd_byte *) obstack_alloc (&objfile->objfile_obstack, length);

...
/* Although scm_t_subr is meant to hold a function pointer, it is
   actually a typedef to "void *".  That means that in C++, an
   explicit cast is necessary to convert function pointer to
   scm_t_subr.  But a cast also makes it possible to pass function
   pointers with the wrong type my mistake.  So instead of adding such
   casts throughout, we use 'scheme_func' to do the conversion, which
   (only) has overloads for function pointer types that are valid.

   https://lists.gnu.org/archive/html/guile-devel/2013-03/msg00001.html
*/

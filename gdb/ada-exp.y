		       ...
		       const char *renaming_expr, int max_depth)
{
  ...
  name = (char *) obstack_copy0 (&temp_parse_space, renamed_entity, renamed_entity_len);
...
{
  struct symbol *sym =
    OBSTACK_ZALLOC (&temp_parse_space, struct symbol);
  SYMBOL_DOMAIN (sym) = UNDEF_DOMAIN;
  SYMBOL_LINKAGE_NAME (sym) = (const char *) obstack_copy0 (&temp_parse_space, name, len);
		   ...
		   const struct block *block, struct stoken name0)
{
  ...
  encoded_name = (char *) obstack_copy0 (&temp_parse_space, encoded_name, name_len);

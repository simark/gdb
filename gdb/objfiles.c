...
get_objfile_pspace_data (struct program_space *pspace)
{
  ...
  info = (struct objfile_pspace_info *) program_space_data (pspace, objfiles_pspace_data);
  if (info == NULL)
    {
...
get_objfile_bfd_data (struct objfile *objfile, struct bfd *abfd)
{
  ...
  if (abfd != NULL)
    storage = (struct objfile_per_bfd_storage *) bfd_data (abfd, objfiles_bfd_data);
  ...
  if (storage == NULL)
	...
	{
	  storage = (struct objfile_per_bfd_storage *) bfd_zalloc (abfd, sizeof (struct objfile_per_bfd_storage));
		       ...
		       const char *name, enum language lang)
{
  if (objfile->per_bfd->name_of_main == NULL
    ...
    objfile->per_bfd->name_of_main
      = (const char *) obstack_copy0 (&objfile->per_bfd->storage_obstack, name, strlen (name));
  ...
  for (debug_objfile = objfile->separate_debug_objfile;
       ...
       debug_objfile = objfile_separate_debug_iterate (objfile, debug_objfile))
    {
	 ...
	 relative ones must be already created according to debug_objfile.  */
      ...
      new_debug_offsets = 
	(struct section_offsets *) xmalloc (SIZEOF_N_SECTION_OFFSETS (debug_objfile->num_sections));

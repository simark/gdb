...
static void mi_on_target_resumed (ptid_t ptid);
...
mi_on_sync_execution_done (void)
{
  ...
  /* MI generally prints a prompt after a command, indicating it's
     ...
     mi_on_target_resumed.  However, if the target is async but MI is
     sync, then we need to output the MI prompt now, to replicate
     gdb's behavior when neither the target nor MI are async.  (Note
     this observer is only called by the asynchronous target event
     handling code.)  */
  if (!mi_async_p ())
    {
...
mi_on_normal_stop (struct bpstats *bs, int print_frame)
{
  ...
  if (print_frame)
      ...
      if (non_stop)
	{
	  ...
	  back_to = make_cleanup_ui_out_list_begin_end (mi->mi_uiout,
							"stopped-threads");
	  ui_out_field_int (mi->mi_uiout, NULL,
			    pid_to_thread_id (inferior_ptid));
	  do_cleanups (back_to);
	}
      else
	ui_out_field_string (mi->mi_uiout, "stopped-threads", "all");
...
mi_tsv_modified (const struct trace_state_variable *tsv)
{
  struct interp *interp = current_interpreter;
  struct mi_interp *mi = interp->data;
  struct ui_out *mi_uiout = interp_ui_out (interp);
...
static void
mi_on_target_resumed (ptid_t ptid)
{
...
mi_solib_loaded (struct so_list *solib)
{
  struct interp *interp = current_interpreter;
  struct mi_interp *mi = interp->data;
  struct ui_out *uiout = interp_ui_out (interp);
